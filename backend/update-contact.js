const { execSync } = require('child_process');

// Função auxiliar para executar comandos SQL via Docker
function executeSQL(sql) {
    const command = `docker exec whatsapp-clone-postgres-1 psql -U whatsapp_user -d whatsapp_clone -t -A -c "${sql.replace(/"/g, '\\"')}"`;

    try {
        const result = execSync(command, { encoding: 'utf-8' });
        return result.trim();
    } catch (error) {
        throw new Error(`SQL Error: ${error.message}`);
    }
}

function updateContact(phone, newName) {
    try {
        // Normalizar telefone (remover caracteres não numéricos)
        const normalizedPhone = phone.replace(/\D/g, '');

        console.log(`🔍 Buscando contato: ${phone} (normalizado: ${normalizedPhone})`);

        // Buscar contato existente
        const checkQuery = `SELECT id, phone, nome FROM contatos WHERE phone = '${normalizedPhone}'`;
        const checkResult = executeSQL(checkQuery);

        if (!checkResult || checkResult.length === 0) {
            console.log(`❌ Contato não encontrado: ${phone} (normalizado: ${normalizedPhone})`);
            return;
        }

        const [id, contactPhone, currentName] = checkResult.split('|');
        console.log(`\n📞 Contato encontrado:`);
        console.log(`   ID: ${id}`);
        console.log(`   Telefone: ${contactPhone}`);
        console.log(`   Nome atual: ${currentName}`);
        console.log(`   Novo nome: ${newName}\n`);

        // Atualizar nome do contato
        const updateContactQuery = `UPDATE contatos SET nome = '${newName.replace(/'/g, "''")}', updated_at = NOW() WHERE phone = '${normalizedPhone}'`;
        executeSQL(updateContactQuery);

        console.log(`✅ Contato atualizado com sucesso!`);
        console.log(`   Nome atualizado para: ${newName}\n`);

        // Atualizar metadata das conversas relacionadas (chatName)
        const chatId = `${normalizedPhone}@s.whatsapp.net`;
        const updateConversationQuery = `
            UPDATE conversas
            SET metadata = jsonb_set(
                COALESCE(metadata, '{}'::jsonb),
                '{chatName}',
                '"${newName.replace(/"/g, '\\"')}"'::jsonb
            ),
            updated_at = NOW()
            WHERE chat_id = '${chatId}'
            RETURNING id
        `.trim();

        try {
            const conversationResult = executeSQL(updateConversationQuery);
            if (conversationResult && conversationResult.length > 0) {
                console.log(`✅ Conversa atualizada!`);
                console.log(`   Chat ID: ${chatId}`);
            } else {
                console.log(`ℹ️  Nenhuma conversa encontrada para este contato`);
            }
        } catch (error) {
            console.log(`ℹ️  Nenhuma conversa encontrada para este contato`);
        }

        console.log(`\n✨ Processo concluído! Recarregue o frontend para ver as mudanças.`);

    } catch (error) {
        console.error('❌ Erro ao atualizar contato:', error.message);
        process.exit(1);
    }
}

// Uso: node update-contact.js "+55 62 9182-5886" "Joao Carlos"
const args = process.argv.slice(2);

if (args.length < 2) {
    console.log('Uso: node update-contact.js <telefone> <novo-nome>');
    console.log('Exemplo: node update-contact.js "+55 62 9182-5886" "Joao Carlos"');
    process.exit(1);
}

const [phone, newName] = args;
updateContact(phone, newName);
